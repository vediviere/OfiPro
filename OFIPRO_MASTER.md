...
La web responsiva mínima vive dentro del mismo repositorio.

Resultado:

Se creó el proyecto frontend Angular dentro del repositorio principal de OfiPro, en la carpeta:
...
No se utilizará un repositorio separado para el frontend por ahora.

---

## D086

Angular será la base de la web responsiva mínima del MVP.

Resultado:

Se creó el frontend con Angular para construir la primera web responsiva mínima de OfiPro.
...
Angular permite construir una web responsiva estructurada, modular y preparada para consumir la API existente.

Regla:

La web responsiva mínima no sustituye la futura app móvil real.

La web servirá para validar flujos principales, facilitar pruebas funcionales y preparar la transición hacia una experiencia mobile-first más completa.

---

## D087

El frontend debe proteger rutas internas mediante JWT, guards y roles.
...
* `authInterceptor` para enviar automáticamente el JWT en peticiones HTTP.
...
OfiPro permite usuarios con más de un rol. Por eso el frontend no debe asumir que un usuario pertenece a un solo modo de operación.

Impacto:

El login puede mandar correctamente al dashboard correspondiente y las rutas internas quedan protegidas contra accesos sin sesión o con rol incorrecto.
...
## D091 El flujo contratista mínimo debe completar el segundo lado del marketplace.

Resultado: El Bloque 20 conecta en Angular el flujo mínimo del contratista:
...
Razón: Después de que el cliente ya podía crear proyectos desde Angular, el siguiente paso necesario era permitir que el contratista interactuara con esos proyectos y enviara propuestas reales.

Impacto: OfiPro deja de tener solamente flujo cliente en frontend. La web ya permite validar el ciclo Cliente publica → Contratista propone.

---

## D092 Las propuestas enviadas deben tener una pantalla propia para el contratista.

Resultado: Se crea la pantalla Mis propuestas para que el contratista pueda consultar las propuestas que ha enviado.

Razón: Enviar una propuesta sin poder verla después genera incertidumbre para el usuario. El contratista necesita confirmar qué envió, por cuánto, en qué tiempo y bajo qué condiciones.

Impacto: El flujo contratista gana trazabilidad básica desde la web responsiva.

---

## D093 El cliente debe revisar propuestas desde el detalle de su proyecto.

Resultado: En el Bloque 21, el detalle del proyecto del cliente muestra las propuestas recibidas por cada requerimiento.

Razón: El cliente no debe revisar propuestas en una pantalla aislada sin contexto. La comparación tiene más sentido dentro del proyecto y requerimiento correspondiente.

Impacto: El cliente puede revisar ofertas recibidas con contexto completo del trabajo publicado.

---

## D094 Aceptar y rechazar propuestas desde frontend debe reutilizar las reglas de negocio del backend.

Resultado: Angular solo consume los endpoints existentes de aceptar/rechazar propuesta. Las reglas importantes permanecen en backend:

* Solo el dueño del proyecto puede aceptar o rechazar.
* Solo propuestas pendientes pueden cambiar de estado.
* Al aceptar una propuesta, el backend genera la contratación correspondiente.
* Al aceptar una propuesta, otras propuestas pendientes del mismo requerimiento se rechazan automáticamente.

Razón: El frontend no debe duplicar reglas críticas de negocio. Debe mostrar acciones y consumir la API, pero la autoridad debe seguir en backend.

Impacto: Se mantiene consistencia entre Swagger, web responsiva y futura app móvil.

---

## D095 El MVP funcional actual no representa la UI/UX final de producción.

Resultado: Se reconoce que los flujos implementados en Bloques 19, 20 y 21 son funcionales, pero todavía no tienen diseño final de producto.

Razón: La prioridad actual es cerrar el ciclo operativo principal del marketplace antes de invertir demasiado en pulido visual.

Impacto: Antes de producción será necesario hacer una pasada completa de UI/UX, especialmente en:
...
JwtService tomaba un solo rol con FirstOrDefault.

Síntoma:

Durante las pruebas del Bloque 18 se detectó que `admin@ofipro.com` era redirigido al dashboard de contratista en lugar del dashboard de administrador.
...
Si un usuario tenía varios roles en `UserRoles`, el token podía generarse con un rol incorrecto o incompleto.

Esto afectaba directamente:

* Redirección por rol en Angular.
* Guards de rol.
* Experiencia de usuarios multirol.
* Pruebas del usuario administrador.
...
* El token de un usuario multirrol contiene todos sus roles.
...
## P050 El contratista no tenía una navegación real hacia proyectos disponibles.

Síntoma: El contratista podía iniciar sesión, pero no tenía todavía un flujo web para ver proyectos publicados por clientes.

Riesgo: El marketplace quedaba incompleto desde frontend, porque el cliente podía publicar pero el contratista no podía responder desde la web.

Solución: Crear pantalla de Proyectos disponibles, agregar rutas protegidas por rol Contratista y mostrar accesos en el menú interno.

Resultado: El contratista puede consultar proyectos disponibles desde Angular.

---

## P051 El contratista podía enviar propuesta, pero necesitaba consultar después lo enviado.

Síntoma: Después de enviar una propuesta, hacía falta una pantalla clara para revisar las propuestas enviadas.

Riesgo: El contratista podía perder trazabilidad de sus ofertas, precio, alcance, tiempos y condiciones.

Solución: Crear pantalla Mis propuestas y mostrar información esencial:
...
Resultado: El contratista puede revisar sus propuestas enviadas desde la web.

---

## P052 El cliente recibía propuestas, pero no podía revisarlas desde Angular.

Síntoma: El backend ya permitía consultar propuestas por requerimiento, aceptar y rechazar, pero el frontend del cliente todavía no mostraba esas propuestas.

Riesgo: El ciclo del marketplace quedaba incompleto desde la web: Cliente publica → Contratista propone, pero Cliente no decide.

Solución: Agregar la visualización de propuestas recibidas dentro del detalle del proyecto del cliente.

Resultado: El cliente puede revisar propuestas recibidas por requerimiento.

---

## P053 El cliente necesitaba acciones directas para aceptar o rechazar propuestas.

Síntoma: Las propuestas podían mostrarse, pero hacía falta accionar sobre ellas desde la web.

Riesgo: El flujo seguía dependiendo de Swagger o pruebas manuales para completar la contratación.

Solución: Agregar botones de Aceptar y Rechazar para propuestas pendientes, consumiendo los endpoints existentes:

* PATCH /api/proposals/{id}/accept
* PATCH /api/proposals/{id}/reject

Resultado: El cliente puede aceptar o rechazar propuestas desde Angular.

---

## P054 Después de aceptar o rechazar una propuesta, la vista debía actualizarse.

Síntoma: Sin recargar propuestas, el usuario podía seguir viendo estados anteriores después de ejecutar una acción.

Riesgo: Confusión visual y posibilidad de intentar acciones sobre propuestas que ya no están pendientes.
...
Web responsiva mínima completada.

Incluye:

* Creación del proyecto Angular en `OfiPro.Web`.
* Limpieza de instalación accidental de Angular fuera del repositorio.
* Corrección de configuración local de Angular y VS Code.
...
* Creación de modelos de autenticación.
* Creación de `AuthService`.
* Implementación de login real.
* Implementación de redirección por rol.
* Implementación de interceptor JWT.
* Implementación de `authGuard`.
* Implementación de `roleGuard`.
* Implementación de logout mínimo.
* Creación de servicio de dashboard.
* Consumo del endpoint protegido `/api/dashboard/me`.
* Corrección de colisión de componentes Dashboard.
* Corrección backend de JWT para usuarios multirrol.
* Validación de login con usuarios Cliente, Contratista y Administrador.
* Validación de rutas protegidas.
* Validación de rutas por rol.
* Validación de logout.
* Validación de consumo de endpoint protegido desde Angular.
...
* Un usuario sin sesión no puede entrar a dashboards.
* Un usuario con sesión expirada vuelve a login.
* Un usuario con rol incorrecto no puede permanecer en rutas no permitidas.
* El administrador tiene prioridad cuando el usuario posee múltiples roles.
* El token JWT se envía automáticamente a endpoints protegidos.
* El logout limpia la sesión local y redirige a login.

Pruebas realizadas:

* Login cliente → redirección a `/cliente/dashboard`.
* Login contratista → redirección a `/contratista/dashboard`.
* Login administrador multirol → redirección a `/admin/dashboard`.
* Acceso manual a ruta protegida sin sesión → redirección a `/login`.
* Acceso manual a ruta no correspondiente al rol → redirección al dashboard permitido.
* Logout desde layout interno → redirección a `/login`.
* Intento de volver al dashboard después de logout → redirección a `/login`.
* Consumo de `GET /api/dashboard/me` desde Angular → 200 OK.
* `dotnet test` → correcto.
* `ng build` → correcto.

Resultado:

OfiPro ya cuenta con una primera web responsiva mínima conectada al backend real.
...
## HITO 20 Frontend: flujo contratista mínimo completado.
Incluye:

* Creación de modelos Angular para propuestas.
* Creación de ProposalService.
* Agregado de método para consultar proyectos disponibles en ProjectService.
* Pantalla Proyectos disponibles.
* Pantalla Detalle de proyecto disponible.
* Formulario para enviar propuesta.
* Pantalla Mis propuestas.
* Registro de rutas protegidas para flujo contratista.
* Ajuste del menú interno para mostrar accesos de contratista.
* Conexión con GET /api/projects para proyectos publicados.
* Conexión con GET /api/projects/{id}.
* Conexión con POST /api/proposals.
* Conexión con GET /api/proposals/my-proposals.
* Visualización de propuestas enviadas por contratista.
* Visualización de campos completos de propuesta:
...
* Solo usuarios con rol Contratista pueden entrar al flujo de proyectos disponibles.
* Solo usuarios con rol Contratista pueden entrar a Mis propuestas.
* El JWT se envía automáticamente al consumir endpoints protegidos.
* El contratista puede enviar propuesta a un requerimiento del proyecto.
...
* Consulta de detalle de proyecto disponible → correcto.
...
Resultado: Bloque 20 completado correctamente. El contratista ya puede consultar proyectos disponibles, enviar propuestas reales y revisar sus propuestas enviadas desde Angular.

Impacto: OfiPro ya tiene conectado el segundo lado operativo del marketplace en frontend.

---

## HITO 21 Frontend: cliente revisa propuestas recibidas completado.

Incluye:

* Agregado de métodos en ProposalService para:
  - Consultar propuestas por requerimiento.
  - Aceptar propuesta.
  - Rechazar propuesta.
	
* Ajuste de ClientProjectDetail para cargar propuestas por cada ProjectRequirement.
* Uso de forkJoin para consultar propuestas de múltiples requerimientos.
* Visualización de propuestas recibidas dentro del detalle del proyecto.
* Visualización de datos del contratista.
* Visualización de precio, tiempo, materiales, garantía, alcance, incluye, no incluye y comentario.
* Botón para aceptar propuesta pendiente.
* Botón para rechazar propuesta pendiente.
* Confirmación básica antes de aceptar o rechazar.
* Manejo de mensajes de éxito.
* Manejo de errores de backend.
* Recarga de propuestas después de aceptar o rechazar.
* Ocultamiento de acciones cuando la propuesta ya no está pendiente.

Endpoints consumidos:

* GET /api/proposals/requirement/{projectRequirementId}
* PATCH /api/proposals/{proposalId}/accept
* PATCH /api/proposals/{proposalId}/reject

Reglas respetadas:

* El frontend no duplica reglas críticas de negocio.
* El backend conserva la autoridad para aceptar, rechazar y generar contratación.
* Solo propuestas pendientes muestran acciones.
* Después de aceptar o rechazar, la vista se actualiza.

Pruebas realizadas:

* Cliente entra a Mis proyectos → correcto.
* Cliente abre detalle de proyecto → correcto.
* Cliente visualiza requerimientos → correcto.
* Cliente visualiza propuestas recibidas → correcto.
* Cliente visualiza datos completos de propuesta → correcto.
* Cliente acepta propuesta desde Angular → correcto.
* Cliente rechaza propuesta desde Angular → correcto.
* La pantalla actualiza estados después de la acción → correcto.
* ng build → correcto.

Resultado: Bloque 21 completado correctamente. El cliente ya puede revisar, aceptar y rechazar propuestas recibidas desde Angular.

Impacto: OfiPro ya cuenta con el ciclo funcional mínimo del marketplace desde la web responsiva:
Cliente crea proyecto → Contratista ve proyecto → Contratista envía propuesta → Cliente revisa propuesta → Cliente acepta o rechaza.
...
La web responsiva mínima ya tiene login, sesión, layouts, dashboards, guards, logout y consumo de endpoint protegido.

El siguiente paso natural es conectar flujos reales del cliente, iniciando con sus proyectos.
...
* Crear proyecto.
* Detalle básico de proyecto.
* Listado de proyectos del cliente consumido desde API.
* Formulario mínimo conectado al backend.
...
* Consulta de proyectos disponibles por contratista.
* Consulta de detalle de proyecto disponible.
* Envío de propuesta desde Angular.
* Consulta de Mis propuestas.
* Visualización completa de propuesta enviada.
* Consulta de propuestas recibidas por cliente.
